...
| `plan_mejora` | FK → PlanMejora | Plan al que pertenece |
| `archivo` | FileField | Archivo almacenado en `media/SoportesPlanes/<plan_id>/` |
| `nombre_original` | CharField(255) | Nombre original del archivo |
| `tipo_soporte` | Choice | EVIDENCIA, ACTA, INFORME, FOTOGRAFIA, PLAN_ACCION, OTRO |
| `descripcion` | TextField | Descripción del soporte |
| `tamano_bytes` | Integer | Tamaño del archivo en bytes |
| `subido_por` | FK → User | Usuario que subió el archivo |
...
### Ejemplo: Subir Soporte

```bash
curl -X POST http://localhost:8000/api/mejoras/planes-mejora/1/soportes/ \
  -H "Authorization: Bearer <token>" \
  -F "archivo=@documento.pdf" \
  -F "tipo_soporte=EVIDENCIA" \
  -F "descripcion=Evidencia de cumplimiento normativo"
```
...
```json
{
  "id": 1,
  "plan_mejora": 1,
  "archivo": "/media/SoportesPlanes/1/a3f2b1c9d4e5_documento.pdf",
  "nombre_original": "documento.pdf",
  "tipo_soporte": "EVIDENCIA",
  "tipo_soporte_display": "Evidencia",
  "descripcion": "Evidencia de cumplimiento normativo",
  "tamano_bytes": 245760,
  "tamano_legible": "240.0 KB",
  "subido_por": 1,
  "fecha_subida": "2026-02-20T10:30:00Z"
}
```
...
```json
{
  "archivo": ["Extensión \".exe\" no permitida. Extensiones válidas: .pdf, .doc, .docx, .png, .jpg, .jpeg, .xls, .xlsx"]
}
```
...
```json
{
  "archivo": ["Tamaño máximo permitido: 10 MB. Archivo recibido: 15.2 MB"]
}
```
...
### Campos Adicionales en Cumplimiento (List)

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `tiene_plan_mejora` | Boolean | `true` si hay plan de mejora vinculado |
| `planes_mejora_count` | Integer | Cantidad de planes vinculados |
| `hallazgos_count` | Integer | Cantidad de hallazgos vinculados |

### Campos Adicionales en Cumplimiento (Detail)

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `planes_mejora_vinculados` | Array | Lista de planes vinculados con id, numero_plan, estado, porcentaje_avance, fecha_vencimiento, vencido |
| `hallazgos_vinculados` | Array | Lista de hallazgos vinculados con id, numero_hallazgo, tipo, severidad, estado |

### Campos Adicionales en Autoevaluación (Detail)

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `planes_mejora_count` | Integer | Total de planes vinculados a la autoevaluación |
| `hallazgos_count` | Integer | Total de hallazgos vinculados |
| `mejoras_resumen` | Object | Resumen con totales por estado de planes y hallazgos |

**Ejemplo `mejoras_resumen`**:
```json
{
  "total_planes": 5,
  "planes_por_estado": {
...
PROGRAMADA → NOTIFICADA → EN_EJECUCION → INFORME → SEGUIMIENTO → CERRADA
                                                                    ↗
                         (cualquier fase) → CANCELADA
```

| Fase | Descripción |
|------|-------------|
| `PROGRAMADA` | Auditoría planificada, con fecha programada |
| `NOTIFICADA` | Equipo auditor y auditados notificados |
| `EN_EJECUCION` | Trabajo de campo en curso |
| `INFORME` | Elaboración del informe final |
| `SEGUIMIENTO` | Seguimiento de acciones correctivas |
| `CERRADA` | Auditoría finalizada |
| `CANCELADA` | Auditoría cancelada (desde cualquier fase) |

### Transiciones Válidas

| Fase Actual | Puede avanzar a |
|-------------|-----------------|
| PROGRAMADA | NOTIFICADA, CANCELADA |
| NOTIFICADA | EN_EJECUCION, CANCELADA |
| EN_EJECUCION | INFORME, CANCELADA |
| INFORME | SEGUIMIENTO, CANCELADA |
| SEGUIMIENTO | CERRADA, CANCELADA |
...
| POST | `/api/audit/auditorias/` | Crear auditoría (fase PROGRAMADA) |
| GET | `/api/audit/auditorias/{id}/` | Detalle con equipo, hallazgos, actas |
...
| DELETE | `/api/audit/auditorias/{id}/eliminar_miembro/` | Quitar miembro del equipo |
| GET | `/api/audit/auditorias/{id}/actas/` | Listar actas de reunión |
| POST | `/api/audit/auditorias/{id}/actas/` | Crear acta de reunión |
| GET | `/api/audit/auditorias/resumen/` | Estadísticas generales |
| GET | `/api/audit/auditorias/proximas/` | Próximas auditorías |
| GET | `/api/audit/auditorias/por-fase/` | Conteos por fase |

### Ejemplo: Crear Auditoría

```bash
curl -X POST http://localhost:8000/api/audit/auditorias/ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "auditoria_nombre": "Auditoría Integral Q1 2026",
    "auditoria_detalle": "Auditoría de procesos de calidad",
...
### Ejemplo: Cambiar Fase

```bash
curl -X POST http://localhost:8000/api/audit/auditorias/1/cambiar-fase/ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"nueva_fase": "NOTIFICADA"}'
```
...
{
  "auditoria_id": 1,
  "fase_anterior": "PROGRAMADA",
  "fase": "NOTIFICADA",
  "mensaje": "Fase cambiada exitosamente"
}
```
...
```json
{
  "nueva_fase": ["No se puede avanzar de 'Informe' a 'PROGRAMADA'. Transiciones permitidas: ['SEGUIMIENTO', 'CANCELADA']"]
}
```

### Modelo Auditoria - Campos Principales

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `auditoria_id` | AutoField (PK) | Identificador único |
| `auditoria_nombre` | CharField(200) | Nombre de la auditoría |
| `auditoria_detalle` | TextField | Objetivo / Alcance |
| `clasificacion` | Choice | INTERNA, EXTERNA |
| `auditoria_tipo` | FK → TipoAuditoria | Tipo de auditoría |
| `auditoria_entidad` | FK → EntidadAuditoria | Entidad auditora |
| `auditoria_proceso` | FK → Process | Proceso auditado |
| `sede` | FK → Headquarters | Sede auditada |
| `norma_referencia` | CharField(200) | Norma de referencia |
| `fase` | Choice | PROGRAMADA → CERRADA |
| `fecha_programada` | DateField | Fecha programada |
| `fecha_inicio_ejecucion` | DateField | Inicio de ejecución |
| `auditor_lider` | FK → User | Líder del equipo auditor |

### Equipo Auditor (MiembroEquipoAuditor)

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `id` | AutoField (PK) | ID del miembro |
| `auditoria` | FK → Auditoria | Auditoría |
| `usuario` | FK → User | Usuario miembro |
| `rol` | Choice | LIDER, AUDITOR, OBSERVADOR, EXPERTO |

## Hallazgos de Auditoría

### Modelo HallazgoAuditoria

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `id` | AutoField (PK) | Identificador |
| `auditoria` | FK → Auditoria | Auditoría origen |
| `numero` | CharField(50) | Código del hallazgo |
| `tipo` | Choice | NC_MAYOR, NC_MENOR, OBSERVACION, OPORTUNIDAD, FORTALEZA |
| `descripcion` | TextField | Descripción del hallazgo |
| `criterio_norma` | CharField | Requisito incumplido |
| `evidencia_objetiva` | TextField | Evidencia encontrada |
| `estado` | Choice | IDENTIFICADO → CERRADO |
| `hallazgo_mejora` | FK → mejoras.Hallazgo | Vinculación con hallazgo transversal |
| `plan_mejora` | FK → mejoras.PlanMejora | Plan de mejora vinculado |

### Endpoints de Hallazgos de Auditoría

| Método | Endpoint | Descripción |
|--------|----------|-------------|
| GET | `/api/audit/hallazgos/` | Listar hallazgos |
...
| GET | `/api/audit/hallazgos/vencidos/` | Hallazgos con fecha límite pasada |
| GET | `/api/audit/hallazgos/estadisticas/` | Estadísticas por tipo/estado |
...
| Método | Endpoint | Descripción |
|--------|----------|-------------|
| GET | `/api/audit/actas/` | Listar actas |
| POST | `/api/audit/actas/` | Crear acta |
| GET | `/api/audit/actas/{id}/` | Detalle |

**Tipos de acta**: `APERTURA`, `CIERRE`, `SEGUIMIENTO`
...
| GET | `/api/audit/programas/{id}/` | Detalle con auditorías |

**Estados del programa**: `BORRADOR`, `APROBADO`, `EN_EJECUCION`, `COMPLETADO`
...
| GET/PUT/DELETE | `/api/audit/tipos/{id}/` | CRUD tipo |
| GET/POST | `/api/audit/entidades/` | Entidades auditoras |
| GET/PUT/DELETE | `/api/audit/entidades/{id}/` | CRUD entidad |
...
|---------|-------|---------|
| 1.0 | 2026-02-18 | Documentación inicial - módulo habilitación |
| 1.1 | 2026-02-19 | Agregado módulo de mejoras (planes, hallazgos) |
| 2.0 | 2026-02-20 | Soportes (upload archivos), integración habilitación↔mejoras, auditoría completa con ciclo de vida |
